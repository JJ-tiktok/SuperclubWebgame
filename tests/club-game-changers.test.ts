...
import { collectPendingGameChangerChoices, isUnusedSecretWeapon, mergeCarriedSecretWeapons, sortPendingGameChangerChoices } from "@/lib/lobby/club-game-changers";
...
describe("club game changer inventory", () => {
  ...
  it("prioritizes offseason comeback choices before other pending rows", () => {
    const playerPick = row({
      id: "player-pick",
      status: "pending",
      created_at: "2026-01-01T00:00:00.000Z",
      choice_payload: { type: "pick_player" },
    });
    const comebackPick = row({
      id: "comeback-pick",
      status: "pending",
      created_at: "2026-01-02T00:00:00.000Z",
      choice_payload: {
        type: "pick_offseason_card",
        candidates: [{ card_id: "c1", display_name: "Bonus A", description: "", category: "good_news", effects: [] }],
      },
    });

    const sorted = sortPendingGameChangerChoices([playerPick, comebackPick]);
    assert.equal(sorted[0]?.id, "comeback-pick");
  });

  it("collects pending rows across seasons", () => {
    const seasonRow = row({
      id: "season-pending",
      status: "pending",
      season_number: 2,
    ...
    const openRow = row({
      ...
      choice_payload: {
        type: "pick_offseason_card",
        candidates: [{ card_id: "c2", display_name: "Bonus B", description: "", category: "secret_weapon", effects: [] }],
      },
    });

    const pending = collectPendingGameChangerChoices([seasonRow], [openRow]);
